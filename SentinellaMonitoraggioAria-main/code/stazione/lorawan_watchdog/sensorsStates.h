...
bool sensorsActiveFlags[] = {true,true,true,false,false,true,true,true}; //This arrays describes for each sensor if we shall read it during read_from_sensor() execution --> true (we read the i sensor) ; false (we skip the reading process for the i sensor)
...
float sensorsAlerts[] = {400.00,40.00,75.00,70.00,7000.00,10.00,10.00}; //critical values for each sensor. //LOW LEVEL
float sensorsCriticalAlerts[] = {500.00,43.00,80.00,80.00,8000.00,12.00,12.00}; //critical values for each sensor. //HIGH LEVEL
