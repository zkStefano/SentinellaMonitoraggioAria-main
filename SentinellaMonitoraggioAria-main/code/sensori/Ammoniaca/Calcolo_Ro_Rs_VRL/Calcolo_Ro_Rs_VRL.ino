/*
   Arduino - Programma che misura il valore di Ro e Rs (e VRL), per una data RL = 47 kohm.
   OSS: Il programma funziona meglio in una stanza con aria fresca, temperatura 20°C, umidità 65%, concentrazione O2 21% (e RL 47 kohm). 
*/

#define RL 47  // la resistenza del resistore vale 47 kohm
void setup()
{
  Serial.begin(9600); 
}
...
void loop() {
  ...
  {
    analog_value = analog_value + analogRead(A0); // somma dei valori anologici letti
  }
  ...
  Rs = ((5.0/VRL)-1) * RL;
  Ro = Rs/3.6; // Rs/Ro vale 3.6 in aria pulita, come si può osservare dal grafico (datasheet MQ137)
